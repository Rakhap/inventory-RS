            ...
            <form wire:submit.prevent="update" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="w-full">
                        ...
                        <x-select-input wire:model.defer="itemForm.room_id">
                            <x-slot:options>
                                @foreach($rooms as $room)
                                    <x-option value="{{$room->id}}" wire:key="{{$room->id}}"
                                    defaultValue="{{$itemForm->room_id}}">{{$room->room_name}}</x-option>
                                @endforeach
                            </x-slot:options>
                        </x-select-input>
                ...
                <div class="flex flex-wrap items-center justify-end gap-6">
                    <x-button type="submit" class="w-max" wire:loading.attr="disabled" wire:target="update">
